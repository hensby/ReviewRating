...
DETAILED_HELP = {
    ...
    'EXAMPLES':
        ...
        file review_rating.py of a debug target (debuggee), run:

          $ {command} review_rating.py:41 --target=<debuggee_id>

        To create a snapshot at line 41 of file review_rating.py on a debug target
        (debuggee) that will only trigger if the variable name has the value of
        'foo', run:

          $ {command} review_rating.py:41 --target=<debuggee_id> --condition="name == 'foo'"

        To create a snapshot at line 41 of file review_rating.py on a debug target
        (debuggee) with the expressions name[0] and name[1], run:

          $ {command} review_rating.py:41 --target=<debuggee_id> --expression="name[0]" --expression="name[1]"
    """
}
