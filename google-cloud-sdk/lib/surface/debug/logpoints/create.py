...
DETAILED_HELP = {
    ...
    'EXAMPLES':
        ...
        variable 'name' at line 41 of file review_rating.py of a debug target (debuggee),
        run:

          $ {command} review_rating.py:41 "Variable name={name}" --target=<debuggee_id>

        To add a logpoint that will print the value of the variable 'name' at
        line 41 of file review_rating.py on a debug target (debuggee) that will only
        trigger if the length of 'name' is greater than 3, run:

          $ {command} review_rating.py:41 "Variable name={name}" --target=<debuggee_id> --condition="len(name) > 3"

        To add a logpoint with a log level of error at line 35 of file review_rating.py
        on a debug target (debuggee), run:

          $ {command} review_rating.py:35 "Unexpected path" --target=<debuggee_id> --log-level=error
    """
}
